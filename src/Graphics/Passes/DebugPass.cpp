...
void RenderPasses::DebugPass(SwapChain& swapChain, DescriptorManagerComponent& dManager,
                             GlobalDSetComponent& globalDSetComponent, PipelineManager& pManager,
                             uint32_t currentFrame, RenderGraph& rg, GeneralManager& gm, GraphicsSettingsComponent& graphicsSettings,
                             ModelManager& mManager)
{
	std::function<void(Entity, std::vector<AABBPush>&)> draw =
	    [&](Entity e, std::vector<AABBPush>& pushData)
	{
	...
	rg.addPass("AABBDebug",
	           {.colorAttachments = {{"MainColor", vk::AttachmentLoadOp::eLoad, vk::AttachmentStoreOp::eStore}},
	            .depthAttachment = RGAttachmentConfig{"Depth", vk::AttachmentLoadOp::eLoad,
