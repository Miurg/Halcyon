...
public:
	...
	void Update(float deltaTime, ComponentManager& cm, const std::vector<Entity>& entities) override
	{
		try
		{
			...
			glm::mat4 projection = glm::perspective(glm::radians(_camera.Fov),
			                                        (GLfloat)*_screenWidth / (GLfloat)*_screenHeight, 0.1f, 1000.0f);
			glUniformMatrix4fv(glGetUniformLocation(_shader.ShaderID, "projection"), 1, GL_FALSE,
